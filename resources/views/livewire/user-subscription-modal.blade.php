<div>
    <div x-data="{ open: false, userId: null }"
         x-on:open-modal.window="if ($event.detail.name === 'edit-user') { open = true; userId = $event.detail.userId; $wire.openModal($event.detail.userId) }"
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div x-show="open"
                 ...
                 class="inline-block align-bottom bg-white dark:bg-dark-2 rounded-md text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                            ...
                            <div class="mt-4 space-y-4">
                                ...
                                <div>
                                    ...
                                    <input type="date" id="starts_at" wire:model.defer="starts_at" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-dark-3 dark:text-gray-200 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                                </div>

                                <div>
                                    <label for="ends_at" class="block text-sm font-medium text-gray-700 dark:text-gray-300">დასრულების თარიღი</label>
                                    <input type="date" id="ends_at" wire:model.defer="ends_at" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-dark-3 dark:text-gray-200 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <!-- Debug info -->
                <div class="px-4 py-2 bg-yellow-50 dark:bg-yellow-900/20 text-xs">
                    <div>User ID: {{ $user?->id ?? 'None' }}</div>
                    <div>Type: {{ $type ?? 'None' }}</div>
                    <div>Starts At: {{ $starts_at ?? 'None' }}</div>
                    <div>Is Active: {{ $is_active ? 'Yes' : 'No' }}</div>
                </div>
            </div>
        </div>
    </div>
