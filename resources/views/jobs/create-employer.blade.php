<x-layout>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div class="bg-white dark:bg-gray-800/50 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700/50 backdrop-blur-sm p-6 mb-8">
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white mb-6">ვაკანსიის შექმნა</h1>
            ...
            <form action="{{ route('employers.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                ...
                <div class="space-y-2">
                    <label for="address" class="block text-sm font-medium text-gray-700 dark:text-gray-300">მისამართი</label>
                ...
                <div class="space-y-2">
                    ...
                    <select name="worktime" id="worktime" 
                        ...
                        @foreach(\App\Enums\WorkTimeType::cases() as $workTimeType)
                            <option value="{{ $workTimeType->value }}" {{ old('work_time_type') == $workTimeType->value ? 'selected' : '' }}>
                                {{ $workTimeType->label() }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="space-y-2">
                    ...
                    <div class="space-y-3">
                        ...
                        <input type="url" name="website" id="website" placeholder="ვებსაიტი (არასავალდებულო)" value="{{ old('website') }}" 
                            class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-primary-500 dark:focus:ring-primary-400 focus:border-transparent shadow-sm transition-all duration-200 text-gray-900 dark:text-white">
                    </div>
                    @error('email')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
