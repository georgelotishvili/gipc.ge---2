<x-layout>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div class="bg-white dark:bg-gray-800/50 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700/50 backdrop-blur-sm p-6 mb-8">
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white mb-6">რეზიუმეს რედაქტირება</h1>
            ...
            <form action="{{ route('employees.update', $employee) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="space-y-2">
                    <label for="name" class="block text-sm font-medium text-gray-700 dark:text-gray-300">სახელი</label>
                ...
                <div class="space-y-2">
                    <label for="salary" class="block text-sm font-medium text-gray-700 dark:text-gray-300">სასურველი ანაზღაურება</label>
                    <input type="number" step="0.01" name="salary" id="salary" value="{{ old('salary', $employee->salary) }}" 
                        class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-primary-500 dark:focus:ring-primary-400 focus:border-transparent shadow-sm transition-all duration-200 text-gray-900 dark:text-white" required>
                    @error('salary')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div class="space-y-2">
                    <label for="description" class="block text-sm font-medium text-gray-700 dark:text-gray-300">გამოცდილება და კვალიფიკაცია</label>
                    <textarea name="description" id="description" rows="5" 
                        class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-primary-500 dark:focus:ring-primary-400 focus:border-transparent shadow-sm transition-all duration-200 text-gray-900 dark:text-white" required>{{ old('description', $employee->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="skills" id="skills" value="{{ old('skills', $employee->skills) }}" 
                        class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-primary-500 dark:focus:ring-primary-400 focus:border-transparent shadow-sm transition-all duration-200 text-gray-900 dark:text-white" required>
                    @error('skills')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div class="space-y-2">
                    ...
                    <div class="space-y-3">
                        <input type="email" name="email" id="email" placeholder="ელ-ფოსტა" value="{{ old('email', $employee->email) }}" 
                            class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-primary-500 dark:focus:ring-primary-400 focus:border-transparent shadow-sm transition-all duration-200 text-gray-900 dark:text-white" required>
                        
                        <input type="text" name="phone" id="phone" placeholder="ტელეფონი" value="{{ old('phone', $employee->phone) }}" 
                            class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-primary-500 dark:focus:ring-primary-400 focus:border-transparent shadow-sm transition-all duration-200 text-gray-900 dark:text-white">
                        
                        <input type="url" name="website" id="website" placeholder="ვებსაიტი (არასავალდებულო)" value="{{ old('website', $employee->website) }}" 
                            class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-primary-500 dark:focus:ring-primary-400 focus:border-transparent shadow-sm transition-all duration-200 text-gray-900 dark:text-white">
                        
                        <input type="text" name="social" id="social" placeholder="სოციალური ქსელები (არასავალდებულო)" value="{{ old('social', $employee->social) }}" 
                            class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-primary-500 dark:focus:ring-primary-400 focus:border-transparent shadow-sm transition-all duration-200 text-gray-900 dark:text-white">
                    </div>
                </div>
                
                <div class="space-y-2">
                    <label for="image" class="block text-sm font-medium text-gray-700 dark:text-gray-300">პროფილის სურათი (არასავალდებულო)</label>
                    @if($employee->image)
                        <div class="mb-3">
                            <img src="{{ $employee->image->url }}" alt="Current profile" class="h-20 w-auto object-contain rounded">
                            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">მიმდინარე სურათი</p>
                        </div>
                    @endif
