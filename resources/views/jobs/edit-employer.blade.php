<x-layout>
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 lg:py-12">
        <!-- Preview Card (full width, on top) -->
        <div class="bg-gradient-to-br from-blue-50/50 via-purple-50/50 to-indigo-50/50 dark:from-blue-900/20 dark:via-purple-900/20 dark:to-indigo-900/20 rounded-2xl sm:rounded-3xl p-4 sm:p-6 lg:p-8 border border-blue-100/50 dark:border-blue-800/30 backdrop-blur-sm">
            <div class="flex items-center mb-4 sm:mb-6">
                <div class="w-8 h-8 sm:w-10 sm:h-10 lg:w-12 lg:h-12 bg-gradient-to-br from-blue-500 via-purple-500 to-indigo-500 rounded-xl sm:rounded-2xl flex items-center justify-center mr-3 sm:mr-4 shadow-lg">
                ...
                <div>
                    <h3 class="text-lg sm:text-xl lg:text-2xl font-bold text-gray-900 dark:text-white">წინასწარი ნახვა</h3>
                    <p class="text-sm sm:text-base text-gray-600 dark:text-gray-400">თქვენი ვაკანსიის ბარათი</p>
                </div>
            </div>
            
            <!-- Job Posting Card -->
            <div class="bg-white dark:bg-gray-800 rounded-xl sm:rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
                <!-- Job Header -->
                <div class="p-4 sm:p-6 border-b border-gray-200 dark:border-gray-700">
                    <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 sm:gap-0">
                        <div class="flex items-center space-x-3 sm:space-x-4">
                            <!-- Company Logo -->
                            <div class="w-12 h-12 sm:w-14 sm:h-14 lg:w-16 lg:h-16 bg-gradient-to-br from-blue-100 to-indigo-100 dark:from-blue-900/30 dark:to-indigo-900/30 rounded-lg sm:rounded-xl border-2 border-gray-200 dark:border-gray-600 overflow-hidden flex items-center justify-center">
                                <img id="preview-logo" src="{{ $employer->image ? $employer->image->url : '' }}" alt="Company Logo" class="w-full h-full object-cover object-center" style="display: {{ $employer->image ? 'block' : 'none' }};">
                                <div id="preview-logo-placeholder" class="w-full h-full flex items-center justify-center" style="display: {{ $employer->image ? 'none' : 'flex' }};">
                                    <svg class="w-6 h-6 sm:w-7 sm:h-7 lg:w-8 lg:h-8 text-gray-400 dark:text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 7v4a1 1 0 001 1h3m10-5h2a2 2 0 012 2v10a2 2 0 01-2 2H6a2 2 0 01-2-2V7a2 2 0 012-2h2"/>
                                    </svg>
                                </div>
                            </div>
                            <div>
                                <h2 id="preview-name" class="text-base sm:text-lg font-semibold text-gray-900 dark:text-white">{{ $employer->name }}</h2>
                                <p class="text-xs sm:text-sm text-gray-500 dark:text-gray-400">კომპანია</p>
                            </div>
                        </div>
                        <!-- Salary Badge -->
                        <div class="bg-green-500 text-white px-3 sm:px-4 py-2 rounded-lg sm:rounded-xl text-center shadow-lg self-start sm:self-auto">
                            <div class="text-xs sm:text-sm font-medium opacity-90">ხელფასი</div>
                            <div class="text-sm sm:text-lg font-bold" id="preview-salary">{{ $employer->salary ? $employer->salary . ' ₾' : '₾ 0' }}</div>
                        </div>
                    </div>
                </div>
                ...
                <div class="p-4 sm:p-6 space-y-4 sm:space-y-6">
                    <!-- Job Position -->
                    <div>
                        <h3 id="preview-position" class="text-lg sm:text-xl lg:text-2xl font-bold text-gray-900 dark:text-white mb-2">{{ $employer->position }}</h3>
                        <div class="flex flex-col sm:flex-row sm:items-center sm:space-x-4 space-y-1 sm:space-y-0 text-xs sm:text-sm text-gray-600 dark:text-gray-400">
                            <span class="flex items-center">
                                <svg class="w-3 h-3 sm:w-4 sm:h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/>
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/>
                                </svg>
                                <span id="preview-address">{{ $employer->address }}</span>
                            </span>
                            <span class="flex items-center">
                                <svg class="w-3 h-3 sm:w-4 sm:h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/>
                                </svg>
                                <span id="preview-worktime">{{ $employer->worktime ? $employer->worktime->label() : 'სრული განაკვეთი' }}</span>
                            </span>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <p id="preview-description" class="text-gray-700 dark:text-gray-300 leading-relaxed">
                            {{ $employer->description ?: 'აქ გამოჩნდება ვაკანსიის აღწერა...' }}
                        </p>
                    </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="flex items-center space-x-3">
                                ...
                                <div>
                                    ...
                                    <p id="preview-email" class="font-medium text-gray-900 dark:text-white">{{ $employer->email ?: 'email@example.com' }}</p>
                                </div>
                            </div>
                            <div class="flex items-center space-x-3">
                                ...
                                <div>
                                    ...
                                    <p id="preview-phone" class="font-medium text-gray-900 dark:text-white">{{ $employer->phone ?: '+995 555 123 456' }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <!-- Gap between preview and form -->
        <div class="mb-16"></div>

        <!-- Form (full width, below preview) -->
        <div class="bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/20 dark:border-gray-700/30 overflow-hidden">
            ...
            <div class="p-8">
                ...
                @endif
                <form action="{{ route('employers.update', $employer) }}" method="POST" enctype="multipart/form-data" class="space-y-10">
                    @csrf
                    @method('PATCH')
                    ...
                    <div class="bg-gradient-to-br from-blue-50/50 via-purple-50/50 to-indigo-50/50 dark:from-blue-900/20 dark:via-purple-900/20 dark:to-indigo-900/20 rounded-3xl p-8 border border-blue-100/50 dark:border-blue-800/30 backdrop-blur-sm">
                        <div class="flex items-center mb-6">
                            ...
                            <div>
                                ...
                                <p class="text-gray-600 dark:text-gray-400">შეასწორეთ კომპანიის ძირითადი მონაცემები</p>
                            </div>
                        </div>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div class="group">
                                <div class="relative">
                                    <input type="text" name="name" id="name" value="{{ old('name', $employer->name) }}" class="w-full px-6 pt-6 pb-2 bg-white/70 dark:bg-gray-800/70 border-2 border-gray-200 dark:border-gray-700 rounded-2xl focus:border-blue-500 dark:focus:border-blue-400 focus:ring-4 focus:ring-blue-500/20 dark:focus:ring-blue-400/20 transition-all duration-300 text-gray-900 dark:text-white placeholder-transparent peer backdrop-blur-sm" placeholder="კომპანიის სახელი" required>
                            ...
                            <div class="group">
                                <div class="relative">
                                    <input type="text" name="address" id="address" value="{{ old('address', $employer->address) }}" class="w-full px-6 pt-6 pb-2 bg-white/70 dark:bg-gray-800/70 border-2 border-gray-200 dark:border-gray-700 rounded-2xl focus:border-blue-500 dark:focus:border-blue-400 focus:ring-4 focus:ring-blue-500/20 dark:focus:ring-blue-400/20 transition-all duration-300 text-gray-900 dark:text-white placeholder-transparent peer backdrop-blur-sm" placeholder="მისამართი" required>
                        ...
                        <div class="group">
                            <div class="relative">
                                <input type="url" name="website" id="website" placeholder="ვებსაიტი (არასავალდებულო)" value="{{ old('website', $employer->website) }}" class="w-full px-6 pt-6 pb-2 bg-white/70 dark:bg-gray-800/70 border-2 border-gray-200 dark:border-gray-700 rounded-2xl focus:border-blue-500 dark:focus:border-blue-400 focus:ring-4 focus:ring-blue-500/20 dark:focus:ring-blue-400/20 transition-all duration-300 text-gray-900 dark:text-white placeholder-transparent peer backdrop-blur-sm">
                            ...
                            <div class="relative">
                                ...
                                <label for="image" class="cursor-pointer block">
                                    ...
                                    <div id="defaultUploadState" class="border-2 border-dashed border-blue-300 dark:border-blue-600 rounded-2xl hover:border-blue-400 dark:hover:border-blue-500 transition-all duration-300 bg-white/50 dark:bg-gray-800/50 backdrop-blur-sm hover:bg-white/70 dark:hover:bg-gray-800/70 p-8 text-center">
                                        @if($employer->image)
                                            <img id="previewImg" src="{{ $employer->image->url }}" alt="Current logo" class="w-24 h-24 object-contain mx-auto mb-4 rounded-xl shadow-lg">
                                        @else
                                    ...
                                    <!-- Image Preview State (JS will toggle) -->
                ...
                <div class="bg-gradient-to-br from-blue-50/50 via-purple-50/50 to-indigo-50/50 dark:from-blue-900/20 dark:via-purple-900/20 dark:to-indigo-900/20 rounded-3xl p-8 border border-blue-100/50 dark:border-blue-800/30 backdrop-blur-sm">
                    <div class="flex items-center mb-6">
                        ...
                        <div>
                            ...
                            <p class="text-gray-600 dark:text-gray-400">შეასწორეთ ვაკანსიის პოზიცია და მოთხოვნები</p>
                        </div>
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div class="group">
                            <div class="relative">
                                <input type="text" name="position" id="position" value="{{ old('position', $employer->position) }}" class="w-full px-6 pt-6 pb-2 bg-white/70 dark:bg-gray-800/70 border-2 border-gray-200 dark:border-gray-700 rounded-2xl focus:border-blue-500 dark:focus:border-blue-400 focus:ring-4 focus:ring-blue-500/20 dark:focus:ring-blue-400/20 transition-all duration-300 text-gray-900 dark:text-white placeholder-transparent peer backdrop-blur-sm" placeholder="პოზიცია" required>
                    ...
                    <div class="mt-8 space-y-8">
                        ...
                        <div class="group">
                            <div class="relative">
                                <input type="text" name="skills" id="skills" value="{{ old('skills', $employer->skills) }}" class="w-full px-6 pt-6 pb-2 bg-white/70 dark:bg-gray-800/70 border-2 border-gray-200 dark:border-gray-700 rounded-2xl focus:border-indigo-500 dark:focus:border-indigo-400 focus:ring-4 focus:ring-indigo-500/20 dark:focus:ring-indigo-400/20 transition-all duration-300 text-gray-900 dark:text-white placeholder-transparent peer backdrop-blur-sm" placeholder="საჭირო უნარები" required>
                ...
                <div class="bg-gradient-to-br from-blue-50/50 via-purple-50/50 to-indigo-50/50 dark:from-blue-900/20 dark:via-purple-900/20 dark:to-indigo-900/20 rounded-3xl p-8 border border-blue-100/50 dark:border-blue-800/30 backdrop-blur-sm">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div class="group">
                            <div class="relative">
                                <input type="email" name="email" id="email" placeholder="ელ-ფოსტა" value="{{ old('email', $employer->email) }}" class="w-full px-6 pt-6 pb-2 bg-white/70 dark:bg-gray-800/70 border-2 border-gray-200 dark:border-gray-700 rounded-2xl focus:border-green-500 dark:focus:border-green-400 focus:ring-4 focus:ring-green-500/20 dark:focus:ring-green-400/20 transition-all duration-300 text-gray-900 dark:text-white placeholder-transparent peer backdrop-blur-sm" required>
                        ...
                        <div class="group">
                            <div class="relative">
                                <input type="text" name="phone" id="phone" placeholder="ტელეფონი" value="{{ old('phone', $employer->phone) }}" class="w-full px-6 pt-6 pb-2 bg-white/70 dark:bg-gray-800/70 border-2 border-gray-200 dark:border-gray-700 rounded-2xl focus:border-green-500 dark:focus:border-green-400 focus:ring-4 focus:ring-green-500/20 dark:focus:ring-green-400/20 transition-all duration-300 text-gray-900 dark:text-white placeholder-transparent peer backdrop-blur-sm" required>
    ...
    <script>
        ...
        function updateEmployerPreview() {
            // Update company name
            const name = document.getElementById('name').value || 'კომპანიის სახელი';
            document.getElementById('preview-name').textContent = name;
            
            // Update position
            const position = document.getElementById('position').value || 'ვაკანსიის პოზიცია';
            document.getElementById('preview-position').textContent = position;
            
            // Update address
            const address = document.getElementById('address').value || 'ქალაქი';
            document.getElementById('preview-address').textContent = address;
            
            // Update salary
            const salary = document.getElementById('salary').value;
            document.getElementById('preview-salary').textContent = salary ? salary + ' ₾' : '₾ 0';
            ...
            // Update description
            const description = document.getElementById('description').value || 'აქ გამოჩნდება ვაკანსიის აღწერა...';
            ...
            if (skills) {
                ...
                if (skillArray.length > 0) {
                    previewSkills.innerHTML = skillArray.map(skill => 
            ...
            // Update phone
            const phone = document.getElementById('phone').value || '+995 555 123 456';
            document.getElementById('preview-phone').textContent = phone;
        }
        
        // Attach event listeners
        ['name', 'position', 'address', 'salary', 'description', 'skills', 'email', 'phone'].forEach(function(id) {
            const el = document.getElementById(id);
            if (el) {
                el.addEventListener('input', updateEmployerPreview);
                el.addEventListener('change', updateEmployerPreview);
            }
        });
        ...
        document.addEventListener('DOMContentLoaded', updateEmployerPreview);
    </script>
</x-layout>