<x-layout>
    <x-workspace-sidebar />
    <div class="min-h-screen bg-slate-900 relative overflow-hidden" x-data="{ showAllVideos: false }">
        <div class="relative z-10 container mx-auto px-4 sm:px-6 lg:px-8 py-2 sm:py-4 lg:py-6">
            <!-- Revolutionary Header -->
            <div class="text-center mb-4 sm:mb-6 lg:mb-8">
                <div class="relative inline-block">
                    <h1 class="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-2 sm:mb-3 lg:mb-4 leading-tight">
                        სასწავლო ვიდეოები
                    </h1>
                </div>
                <p class="text-sm sm:text-base md:text-lg lg:text-xl text-gray-300 max-w-md sm:max-w-2xl lg:max-w-4xl mx-auto leading-relaxed font-light px-4">
            ...
            <div class="flex justify-center mb-4 sm:mb-6 lg:mb-8">
                <div class="relative bg-black/20 backdrop-blur-xl p-1.5 sm:p-2 rounded-2xl lg:rounded-3xl border border-white/10 shadow-2xl max-w-full">
                    <div class="flex items-center relative z-10">
                        <button 
                            @click="showAllVideos = false" 
                            :class="{'bg-gradient-to-r from-cyan-500 to-purple-500 text-white shadow-lg scale-105': !showAllVideos, 'text-gray-400 hover:text-white': showAllVideos}"
                        ...
                        <button 
                            ...
                            :class="{'bg-gradient-to-r from-purple-500 to-pink-500 text-white shadow-lg scale-105': showAllVideos, 'text-gray-400 hover:text-white': !showAllVideos}"
                            class="px-4 sm:px-6 lg:px-8 py-2.5 sm:py-3 lg:py-4 rounded-xl lg:rounded-2xl font-bold transition-all duration-500 flex items-center gap-2 lg:gap-3 relative overflow-hidden group text-sm lg:text-base"
                        >
            ...
            <div x-show="!showAllVideos" x-transition class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 lg:gap-6 mb-6 sm:mb-8">
                ...
                <!-- Revolutionary Course Card -->
                    <a href="{{ route('tutorials.chapters', $course->id) }}" class="group bg-white/8 backdrop-blur-xl rounded-2xl sm:rounded-3xl overflow-hidden shadow-xl hover:shadow-2xl transition-all duration-700 border border-white/15 hover:border-white/25 h-full flex flex-col relative">
                        ...
                        <div class="h-32 sm:h-36 lg:h-40 relative overflow-hidden">
                            ...
                            @else
                                <div class="w-full h-full bg-gradient-to-br from-white/8 to-white/4 flex items-center justify-center relative">
                                    <div class="absolute inset-0 bg-gradient-to-r from-blue-500/10 via-purple-500/10 to-indigo-500/10 animate-pulse"></div>
                                    <div class="text-center relative z-10">
                                        <div class="w-12 h-12 sm:w-16 sm:h-16 lg:w-20 lg:h-20 bg-white/15 backdrop-blur-sm rounded-2xl sm:rounded-3xl flex items-center justify-center mb-2 sm:mb-3 lg:mb-4 group-hover:scale-110 transition-transform duration-700 border border-white/20 shadow-2xl">
                                            <i class="fas fa-graduation-cap text-lg sm:text-2xl lg:text-3xl text-white"></i>
                                        </div>
                                        <span class="text-white/90 font-medium text-xs sm:text-sm lg:text-base">სასწავლო კურსი</span>
                                    </div>
                                </div>
                            @endif
                        ...
                        <div class="p-2 sm:p-3 lg:p-4 flex-1 flex flex-col relative">
                            ...
                            <div class="mb-2 sm:mb-3 flex-1">
                                <p class="text-gray-300 leading-relaxed line-clamp-2 text-xs sm:text-sm font-light group-hover:text-gray-200 transition-colors duration-500">
                                    {{ $course->description }}
                                </p>
                            </div>
                            ...
                            <div class="mb-2 sm:mb-3">
                                <div class="grid grid-cols-2 gap-2 sm:gap-3">
                                    <div class="bg-gradient-to-br from-white/8 to-white/4 backdrop-blur-sm rounded-lg sm:rounded-xl p-2 sm:p-3 border border-white/10 group-hover:bg-gradient-to-br group-hover:from-blue-500/10 group-hover:to-blue-600/10 transition-all duration-500 h-12 sm:h-16 flex items-center relative overflow-hidden">
                                        ...
                                        <div class="flex items-center gap-1.5 sm:gap-2.5 w-full relative z-10">
                                            ...
                                            <div class="min-w-0 flex-1">
                                                <div class="text-white font-bold text-sm sm:text-base group-hover:text-blue-300 transition-colors duration-300">{{ $course->chapters->sum(function($chapter) { return $chapter->videos->count(); }) }}</div>
                                                <div class="text-gray-400 text-xs font-medium">ვიდეო</div>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="bg-gradient-to-br from-white/8 to-white/4 backdrop-blur-sm rounded-lg sm:rounded-xl p-2 sm:p-3 border border-white/10 group-hover:bg-gradient-to-br group-hover:from-purple-500/10 group-hover:to-purple-600/10 transition-all duration-500 h-12 sm:h-16 flex items-center relative overflow-hidden">
                                        ...
                                        <div class="flex items-center gap-1.5 sm:gap-2.5 w-full relative z-10">
                                            ...
                                            <div class="min-w-0 flex-1">
                                                ...
                                                @endphp
                                                <div class="text-white font-bold text-sm sm:text-base group-hover:text-purple-300 transition-colors duration-300">
                                                    @if($hours > 0)
                                                        {{ $hours }}+ საათი
                                                    @else
                                                        {{ $minutes }} წთ
                            ...
                            <div class="mb-2 sm:mb-3">
                                <div class="flex items-center justify-between text-xs text-gray-400">
                                    <span class="flex items-center gap-1 sm:gap-1.5 group-hover:text-gray-300 transition-colors duration-300">
                                        <i class="fas fa-layer-group text-white/60 group-hover:text-blue-400 transition-colors duration-300"></i>
                                        <span class="hidden sm:inline">{{ $course->chapters->count() }} თავი</span>
                                        <span class="sm:hidden">{{ $course->chapters->count() }}</span>
                                    </span>
                                    <span class="flex items-center gap-1 sm:gap-1.5 group-hover:text-gray-300 transition-colors duration-300">
                                        <i class="fas fa-users text-white/60 group-hover:text-purple-400 transition-colors duration-300"></i>
            ...
            <!-- Video Grid -->
            <div x-show="showAllVideos" x-cloak x-transition class="flex flex-wrap gap-3 sm:gap-4 lg:gap-6 justify-center">
                @foreach ($courses as $course)
