        ...
        <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 lg:gap-6">
            ...
            <div class="flex flex-col md:flex-row items-start md:items-center md:justify-between lg:justify-normal gap-3 md:gap-6 w-full lg:w-auto order-last lg:order-none">
                <div class="relative inline-block text-left w-full md:w-auto my-2">
                    <a href="{{route('admin.questions.create')}}" class="px-4 py-3 bg-primary-600 text-white rounded-md hover:bg-primary-700 transition-colors duration-200">
                        დამატება
                    </a>
                </div>
                ...
                <livewire:question-search />
                    <div class="relative inline-block text-left w-full md:w-auto">
                        ...
                        <div id="filter-dropdown" class="hidden absolute right-0 mt-2 w-64 rounded-md shadow-lg bg-white dark:bg-dark-2 border border-gray-100/50 dark:border-dark-4/50 overflow-hidden z-50">
                                    ...
                                    @foreach ($groups as $group)
                                        <a href="{{ route('admin.questions', ['g' => $group->name]) }}" 
                                            ...
                                            <div class="flex items-center gap-3">
                                                <div class="h-7 w-7 rounded-md {{ request('g') == $group->name 
                                                    ? 'bg-white/20' 
                    ...
                    <div class="flex gap-2">
                        <button id="deleteButton" onclick="toggleDeleteMode()" class="w-full md:w-auto px-4 py-2.5 bg-red-600 text-white rounded-md hover:shadow-lg transition-all duration-300">
                            არჩეულის წაშლა
                        </button>
                    </div>
                </div>
        ...
        <form id="deleteForm" action="{{route('admin.questions.destroy', 'bulk')}}" method="POST">
            ...
            <div class="space-y-4">
                @foreach($questions as $question)
                    <div class="relative group bg-white dark:bg-dark-2 rounded-md border-2 border-gray-100 dark:border-dark-4 overflow-hidden transition-all duration-300 hover:shadow-[0_15px_30px_-15px_rgba(0,0,0,0.1)] dark:hover:shadow-[0_15px_30px_-15px_rgba(0,0,0,0.3)] hover:border-primary/30 dark:hover:border-primary/30">
                        <div class="p-4 flex flex-col md:flex-row justify-between items-start md:items-center space-y-2 md:space-y-0 md:space-x-4">
                            <div class="flex items-center">
                                ...
                                <div class="flex flex-col md:flex-row items-start md:items-center w-full md:w-auto space-y-2 md:space-y-0 md:space-x-6">
                                    <div class="flex justify-between w-full md:w-auto items-center space-x-4">
                                        ...
                                        <span class="px-3 py-1.5 text-xs font-medium rounded-md bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border-2 border-indigo-100/50 dark:border-indigo-500/20">
                            ...
                            <div class="flex justify-end mt-3 md:mt-0 space-x-2">
                                <a href="{{route('admin.questions.edit', $question->id)}}" 
                                    class="p-2.5 text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300 rounded-md hover:bg-indigo-50 dark:hover:bg-indigo-500/10 transition-all duration-300 hover:-translate-y-0.5">
                                ...
                                <form action="{{route('admin.questions.destroy', $question->id)}}" method="POST" class="inline">
                                    ...
                                    @csrf
                                    <button type="submit" 
                                            class="p-2.5 text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-300 rounded-md hover:bg-red-50 dark:hover:bg-red-500/10 transition-all duration-300 hover:-translate-y-0.5">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
