        ...
        <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm overflow-hidden">
                ...
                <div class="p-6">
                    ...
                    <form action="{{ route('admin.plan-options.store') }}" method="POST">
                        @csrf
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                             ...
                             <div>
                                ...
                                <input type="text" 
                                       ...
                                       value="{{ old('option_description') }}"
                                         placeholder="მაგ: უფასო კონსულტაცია"
                                       class="w-full rounded-md border border-gray-300 dark:border-gray-700 dark:bg-dark-3 px-4 py-2 focus:border-primary-500 focus:ring-primary-500">
                            ...
                            <div>
                                ...
                                <select name="is_included" id="is_included"
                                    class="w-full rounded-md border border-gray-300 dark:border-gray-700 dark:bg-dark-3 px-4 py-2 focus:border-primary-500 focus:ring-primary-500">
                                    <option disabled selected>გამოჩნდეს?</option>
                                    <option value="1" {{ old('is_included') == 1 ? 'selected' : '' }}>დიახ</option>
                                    <option value="0" {{ old('is_included') == 0 ? 'selected' : '' }}>არა</option>
                                </select>
                                @error('is_included')
                                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <select name="is_active" id="is_active"
                                    ...
                                    <option value="1" {{ old('is_active') == 1 ? 'selected' : '' }}>აქტიური</option>
                                    <option value="0" {{ old('is_active') == 0 ? 'selected' : '' }}>გათიშული</option>
                                </select>
                                @error('is_active')
                                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
