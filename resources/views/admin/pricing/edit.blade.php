                    ...
                    <form action="{{ route('admin.pricing.update', $pricing->id) }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select name="plan_id" id="plan_id"
                                    ...
                                    @foreach (App\Models\Plan::all() as $plan)
                                        <option value="{{ $plan->id }}" {{ old('plan_id', $pricing->plan_id) == $plan->id ? 'selected' : '' }}>
                                            {{ $plan->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <input type="text" name="name" id="name" value="{{ $pricing->name }}" required
                                    class="w-full rounded-md border border-gray-300 dark:border-gray-700 dark:bg-dark-3 px-4 py-2 focus:border-primary-500 focus:ring-primary-500">
                                @error('name')
                                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <input type="number" name="price" id="price" value="{{ $pricing->price }}" required
                                    class="w-full rounded-md border border-gray-300 dark:border-gray-700 dark:bg-dark-3 px-4 py-2 focus:border-primary-500 focus:ring-primary-500">
                                @error('price')
                                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <input type="text" name="small_description" id="small_description"
                                    value="{{ $pricing->small_description }}"
                                    class="w-full rounded-md border border-gray-300 dark:border-gray-700 dark:bg-dark-3 px-4 py-2 focus:border-primary-500 focus:ring-primary-500">
                            ...
                            <div>
                                ...
                                <select name="featured" id="featured"
                                    class="w-full rounded-md border border-gray-300 dark:border-gray-700 dark:bg-dark-3 px-4 py-2 focus:border-primary-500 focus:ring-primary-500">
                                    <option disabled {{ old('featured', $pricing->featured) ? '' : 'selected' }}>იყოს
                                        რეკომენდებული?
                                    </option>
                                    <option value="true" {{ old('featured', $pricing->featured) == 'true' ? 'selected' : '' }}>კი</option>
                                    <option value="false" {{ old('featured', $pricing->featured) == 'false' ? 'selected' : '' }}>არა</option>
                                </select>
                                @error('featured')
                                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                            ...
                            <!-- Tags -->
                            <div x-data="{
                                tags: {{ json_encode(explode(',', $pricing->tags)) }},
                                input: '',
                                addTag() {
