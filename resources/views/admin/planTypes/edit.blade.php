        ...
        <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 rounded-md shadow-sm overflow-hidden">
                <div class="p-6 border-b border-gray-200 dark:border-gray-700">
                    <h3 class="text-2xl font-bold text-gray-800 dark:text-white">ტიპის რედაქტირება</h3>
                </div>
                <div class="p-6">
                    @if(session('error'))
                        <div
                    ...
                    <form action="{{ route('admin.plan-types.update', $planType->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input type="text" name="type_name" id="type_name" value="{{ $planType->type_name }}" required placeholder="მაგ: 1 თვე"
                            ...
                            <div>
                                ...
                                <input type="number" name="type_duration" id="type_duration" value="{{ $planType->type_duration }}" required placeholder="მაგ: 7, 30, 365"
                                    class="w-full rounded-md border border-gray-300 dark:border-gray-700 dark:bg-dark-3 px-4 py-2 focus:border-primary-500 focus:ring-primary-500">
                                @error('type_duration')
                                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
