<aside class="fixed left-0 top-0 z-100 flex h-screen flex-col overflow-y-hidden bg-white dark:bg-dark"
    ...
    <button @click="sidebarOpen = !sidebarOpen" class="absolute top-20" :class="{ 'right-7': !sidebarOpen, 'right-2': sidebarOpen }">
        <svg :class="{ 'rotate-180': sidebarOpen }" class="w-6 h-6 text-gray-500 group-hover:text-primary-600 transition-colors duration-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
        </svg>
    </button>
    <!-- Navigation -->
    <nav class="flex-1 mt-24 overflow-y-auto overflow-x-hidden py-4">
        <!-- Menu Group -->
        <div class="mb-6">
                ...
                <li>
                    <a href="" wire:navigate
                       ...
                       {{ Route::is('admin.questions') ? 'text-primary-600 bg-primary/[0.08] dark:bg-primary/[0.03] border-r-4 border-primary-600' : 'text-gray-700 dark:text-dark-6' }}">
                        ...
                        <span :class="{ 'opacity-0 hidden': !sidebarOpen }">წინასაგამოცდო ტესტირება</span>
                    </a>
                </li>
